...
def update_progress():
    ...
    while True:
        if pygame.mixer.get_busy() and not paused:
            current_position = pygame.mixer.music.get_pos() / 1000
            pbar["value"] = current_position

            # Check if the current song has reached its maximux duration
    ...
    if selected_folder_path:
        ...
        for filename in os.listdir(selected_folder_path):
            if filename.endswith(".mp3"):
                ...
                # Insert only the filename, not the full path
...
def play_selected_song():
    if len(lbox.curselection()) > 0:
        ...
        # Set the maximum value of the pbar to the song duration
...
# Create a list to display the available songs
...
# Create a button to go to the next music
btn_next = ctk.CTkButton(btn_frame, text=">",
                         command=next_music, width=50, font=("TkDefaultFont", 18))
